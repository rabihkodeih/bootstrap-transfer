(function($){
	/**
	 * Replace variables to values. Example: '%avaible%' => 'Avaible'
	 * @param template
	 * @param variables
	 * @return {*}
	 */
	var replaceTemplateVariables = function(template, variables) {
		for (i in variables) {
			...
			template = template.replace(expr, variables[i]);
		}
		return template;
	};
	...
	$.fn.bootstrapTransfer = function(options) {
		...
		return this.each(function(){
			...
			var template = replaceTemplateVariables(settings.template, settings.templateVars);
	...
	$.fn.bootstrapTransfer.defaults = {
		templateVars: {
			...
			'%clear all%': 'Clear all',
			'%remove%': 'Remove',
			'%select then click%': 'Select then click'
		},
		onUpdate: false,
		template:
			'<table width="92%" cellspacing="0" cellpadding="0" style="margin-left:4%">\
<tr>\
<td width="50%">\
...
<a href="javascript:void(0)" class="selector-chooseall">%choose all%</a>\
</div>\
</td>\
<td>\
<div class="selector-chooser">\
<a href="javascript:void(0)" class="selector-add">%add%</a>\
<a href="javascript:void(0)" class="selector-remove">%remove%/a>\
</div>\
</td>\
<td width="50%">\
...
<div><div class="illustration"></div><div class="click-text">%select then click%</div></div>\
</div>\
<select multiple="multiple" class="filtered target">\
</select>\
<a href="javascript:void(0)" class="selector-clearall">%clear all%</a>\
</div>\
</td>\
</tr>\
</table>',
